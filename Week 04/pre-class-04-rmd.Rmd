...
#First we create survey.clean for us to be working on. Within this dataset, we move through each row (j) and each column (i) and say that if it is below 1 or above , change its value to NA. Then we print the new dataset out
...
survey3b <- survey.clean #creating dataframe to work off of
...
print(survey3b) #check it worked
  ...
  #I go row by row (each row is a participant) and look at columns 2-6 (i.e. q1 to q5) to add up any missing values. I  toal up these values (i.e. how many NA's there are) and store this as a vector called invalid.vector. I then add this vector as a column in the dataframe, change its name to the requested name, and glimpse it to check my totals worked.
